...
export function RankingTableColumnFilters({ columns, onColumnChange }: ColumnSelectorProps) {
  const handleCheckboxChange = (id: keyof ColumnOptions, checked: boolean) => {
        ...
        <SimpleGrid cols={2} spacing="sm" breakpoints={[{ maxWidth: 'sm', cols: 1 }]}>
          {columnItems.map((item) => (
            <Checkbox
              ...
              onChange={(e) => handleCheckboxChange(item.id as keyof ColumnOptions, e.currentTarget.checked)}
            />
          ))}
        </SimpleGrid>
      </Menu.Dropdown>

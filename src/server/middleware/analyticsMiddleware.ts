...
const analyticsQuerySchema = z.object({
    ...
    includeInactive: z.string().optional().default('false').transform((val: string) => val === 'true'),
    timeframe: z.enum(['current', 'daily']).optional().default('current'),
    minimumGames: z.string().optional().transform((val?: string) => val ? parseInt(val, 10) : 20).refine((val: number) => val >= 0 && val <= 1000, {
...
const activityAnalysisQuerySchema = z.object({
    includeInactive: z.string().optional().default('false').transform((val: string) => val === 'true'),
    groupBy: z.enum(['race', 'league', 'activity']).optional().default('activity'),
    timeframe: z.enum(['current', 'daily']).optional().default('current'),
    minimumGames: z.string().optional().transform((val?: string) => val ? parseInt(val, 10) : 20).refine((val: number) => val >= 0 && val <= 1000, {
        message: "Minimum games must be between 0 and 1000"
    })
...
export const expensiveAnalyticsRateLimit = rateLimit({
    ...
    keyGenerator: (req: Request) => {
        ...
        return `expensive-${req.ip || req.socket?.remoteAddress || 'unknown'}`
    },
    skip: (_req: Request) => {
        return process.env.NODE_ENV === 'test'
    }
...
export const requireAnalyticsFeature = (req: Request, res: Response, next: NextFunction) => {
    const analyticsEnabled = String(process.env.ENABLE_PLAYER_ANALYTICS ?? 'false').toLowerCase() === 'true'
...
export const analyticsSecurityHeaders = (_req: Request, res: Response, next: NextFunction) => {
...
export const analyticsBodyLimit = (req: Request, res: Response, next: NextFunction) => {
...
export const analyticsPerformanceMonitoring = (req: Request, res: Response, next: NextFunction) => {
